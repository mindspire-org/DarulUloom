...
if (missingEnvVars.length > 0) {
  console.error(
    `❌ Critical environment variables missing: ${missingEnvVars.join(", ")}`,
  );
...
const corsOptions = {
  origin: (origin, callback) => {
    ...
    const lanIpPattern =
      /^https?:\/\/(10\.\d{1,3}\.\d{1,3}\.\d{1,3}|172\.(1[6-9]|2[0-9]|3[0-1])\.\d{1,3}\.\d{1,3}|192\.168\.\d{1,3}\.\d{1,3})(:\d+)?$/;
...
app.listen(PORT, "0.0.0.0", () => {
  ...
  if (lanIp) {
    ...
    console.log(
      "   - LAN: Could not determine local IP address. Check network connection.",
    );
  }
